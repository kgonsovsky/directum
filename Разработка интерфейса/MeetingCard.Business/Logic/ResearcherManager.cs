...
{
    public class ResearcherManagerBl
    {
    
        private static ResearcherManagerBl _instance = null;

        public List<ResearcherBl> ResearcherList => GetResearchersList();

        public static ResearcherManagerBl Instance()
        {
            return _instance ?? (_instance = new ResearcherManagerBl());
        }

        public ResearcherBl GetResearcher(int researcherId)
        {
        ...
        public void UpdateResearcher(ResearcherBl researcher)
        {
            var manager = new FactoryManager();
        ...
        private List<ResearcherBl> GetResearchersList()
        {
            var researchers = new List<ResearcherBl>();
            var manager = new FactoryManager();
            foreach (var researcher in manager.GetResearcherManager().FindAll())
            {
                var res = new ResearcherBl(researcher.Id, researcher.LastName, researcher.FirstName,
                    researcher.MiddleName, researcher.DepartmentNumber, researcher.Age, researcher.AcademicDegree, researcher.Position);
                researchers.Add(res);
            }
            return researchers;
