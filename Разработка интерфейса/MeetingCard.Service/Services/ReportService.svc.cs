...
using MeetingCard.Service.IServices;

namespace MeetingCard.Service.Services
{
    ...
    public class ReportService : IReportService
    {
        public List<Report> GetReportByResearcher(int researcherId)
        {
            ...
            foreach (var report in Business.Logic.ResearcherManagerBl.Instance().GetResearcher(researcherId).Reports)
            {
        ...
        {
            return Business.Logic.ResearcherManagerBl.Instance().GetResearcher(researcherId).AddReport(report.Name,
                report.RegisterNumber, report.ReleaseYear, report.PageCount);
        }

        public void UpdateReport(Report report)
        {
            new Business.Logic.ReportBl(report.Id).Researcher.UpdateReport(new Business.Logic.ReportBl(report.Id,
                report.Name, report.RegisterNumber, report.ReleaseYear, report.PageCount));
        }

        public void DeleteReport(int reportId)
        {
            new Business.Logic.ReportBl(reportId).Researcher.DeleteReport(reportId);
        }

        public Report GetReport(int reportId)
        {
            var report = new Business.Logic.ReportBl(reportId);
            report.Refresh();
